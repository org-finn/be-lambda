...
def call_gemini_model(articles):
    ...
    1. 위 뉴스들을 종합적으로 분석하여 긍정적인 요인과 부정적인 요인을 각각 파악하세요. (한글로 작성)
        1-1. 'positiveReasoning': 긍정적인 요인들을 최대 3개 추출하세요. 각 문장은 엔터 단위로 구분해서 해당 필드에 모두 담아주세요. 각 문장의 길이는 최대 30자로 제한해주세요. (해당 내용이 없으면 null 반환)
        1-2. 'negativeReasoning': 부정적인 요인들을 최대 3개 추출하세요. 각 문장은 엔터 단위로 구분해서 해당 필드에 모두 담아주세요. 각 문장의 길이는 최대 30자로 제한해주세요. (해당 내용이 없으면 null 반환)
